...
/**
 * @title SUPCVesting
 * @dev A SUPC holder contract that can release its token balance gradually like a
 * typical vesting scheme, with a cliff and vesting period. Optionally revocable by the
 * owner.
 */
contract SUPCVesting is Ownable(msg.sender) {
    ...
    constructor (address beneficiary, uint256 start, uint256 cliffDuration, uint256 duration, uint256 unlockPCT, bool revocable) {
        require(beneficiary != address(0), "SUPCVesting::constructor: beneficiary is the zero address");
        // solhint-disable-next-line max-line-length
        require(cliffDuration <= duration, "SUPCVesting::constructor: cliff is longer than duration");
        require(duration > 0, "SUPCVesting::constructor: duration is 0");
        // solhint-disable-next-line max-line-length
        require(start + duration > block.timestamp, "SUPCVesting::constructor: final time is before current time");
    ...
    function release(IERC20 token) public {
        ...
        require(unreleased > 0, "SUPCVesting::release: no tokens are due");
    ...
    function releaseToAddress(IERC20 token, address receiver, uint256 amount) public {
        require(_msgSender() == _beneficiary, "SUPCVesting::setBeneficiary: Not contract beneficiary");
        require(amount > 0, "SUPCVesting::_releaseToAddress: amount should be greater than 0");

        require(receiver != address(0), "SUPCVesting::_releaseToAddress: receiver is the zero address");

        uint256 unreleased = _releasableAmount(token);

        require(unreleased > 0, "SUPCVesting::_releaseToAddress: no tokens are due");

        require(unreleased >= amount, "SUPCVesting::_releaseToAddress: enough tokens not vested yet");
    ...
    function revoke(IERC20 token) public onlyOwner {
        require(_revocable, "SUPCVesting::revoke: cannot revoke");
        require(!_revoked[address(token)], "SUPCVesting::revoke: token already revoked");
    ...
    function setBeneficiary(address newBeneficiary) public {
        require(_msgSender() == _beneficiary, "SUPCVesting::setBeneficiary: Not contract beneficiary");
        require(_beneficiary != newBeneficiary, "SUPCVesting::setBeneficiary: Same beneficiary address as old");
